//To test compile:
//gcc -c iir.c -O2 -std=c99 -Wall -Wextra
//clang -c iir.c -O2 -std=c99 -Weverything
//g++ -c iir.c -O2 -std=c++11 -Wall -Wextra
//clang++ -c iir.c -O2 -std=c++11 -Weverything
...
int iir_s (float *X, const char iscolmajor, const int R, const int C, const float *A, const int N, const int dim)
{
    ...
    //Checks
    if (R<1) { fprintf(stderr,"error in iir_s: R (nrows X) must be positive\n"); return 1; }
    if (C<1) { fprintf(stderr,"error in iir_s: C (ncols X) must be positive\n"); return 1; }
    if (N<1) { fprintf(stderr,"error in iir_s: N (filter order) must be positive\n"); return 1; }
    ...
    if (dim==0)
    {
        if (C==1)
        {
            ...
            for (n=M; n<R; n++) { X[n] -= cblas_sdot(M,&A[0],1,&X[n-M],1); }
        }
        else if (C==2)
        {
            if (iscolmajor)
