...
descr += "This is the same as L, the number of AC lags used to compute.\n";
descr += "Note that this includes the trivial 0th coeff (ones).\n";
descr += "\n";
...
descr += "\n";
descr += "Examples:\n";
descr += "$ lin_pred -p3 X -o Y -o V \n";
descr += "$ lin_pred -d1 -p5 X -o Y -o V \n";
descr += "$ cat X | lin_pred -p7 -m > Y \n";

//Argtable
struct arg_file  *a_fi = arg_filen(nullptr,nullptr,"<file>",I-1,I,"input file (X)");
struct arg_int    *a_p = arg_intn("p","P","<uint>",0,1,"number of LP coeffs [default=2]");
...
if (i1.T==1)
{
    ...
    if (ov::lin_pred_s(Y,V,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,P,m0)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
    if (wo1)
    {
