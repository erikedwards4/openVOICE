...
//To test compile:
//gcc -c ac2cc.c -O2 -std=c99 -Wall -Wextra
//clang -c ac2cc.c -O2 -std=c99 -Weverything
//g++ -c ac2cc.c -O2 -std=c++11 -Wall -Wextra
//clang++ -c ac2cc.c -O2 -std=c++11 -Weverything
...
int ac2cc_s (float *CC, const float *AC, const char iscolmajor, const int R, const int C, const int dim, const int K, const float preg)
{
    ...
    //Checks
    if (dim==0 && P>R) { fprintf(stderr,"error in ac2cc_s: P must be < nrows AC for dim==0\n"); return 1; }
    if (dim==1 && P>C) { fprintf(stderr,"error in ac2cc_s: P must be < ncols AC for dim==1\n"); return 1; }
	
    //Initialize
    if (!(AS=(float *)malloc((size_t)P*sizeof(float)))) { fprintf(stderr,"error in ac2cc_s: problem with malloc. "); perror("malloc"); return 1; }
    if (!(AStmp=(float *)malloc((size_t)(P-1)*sizeof(float)))) { fprintf(stderr,"error in ac2cc_s: problem with malloc. "); perror("malloc"); return 1; }
    ...
    {
        fprintf(stderr,"error in ac2cc_s: dim must be 0 or 1.\n"); return 1;
    }
...
int ac2cc_d (double *CC, const double *AC, const char iscolmajor, const int R, const int C, const int dim, const int K, const double preg)
{
    ...
    //Checks
    if (dim==0 && P>R) { fprintf(stderr,"error in ac2cc_d: P must be < nrows AC for dim==0\n"); return 1; }
    if (dim==1 && P>C) { fprintf(stderr,"error in ac2cc_d: P must be < ncols AC for dim==1\n"); return 1; }
	
    //Initialize
    if (!(AS=(double *)malloc((size_t)P*sizeof(double)))) { fprintf(stderr,"error in ac2cc_d: problem with malloc. "); perror("malloc"); return 1; }
    if (!(AStmp=(double *)malloc((size_t)(P-1)*sizeof(double)))) { fprintf(stderr,"error in ac2cc_d: problem with malloc. "); perror("malloc"); return 1; }
    ...
    {
        fprintf(stderr,"error in ac2cc_d: dim must be 0 or 1.\n"); return 1;
    }
