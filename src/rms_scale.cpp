...
int main(int argc, char *argv[])
{
    ...
    void *argtable[] = {a_fi, a_fs, a_d, a_tau, a_spl, a_max, a_fo, a_help, a_end};
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::rms_scale_d(X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,double(fs),double(tau),double(spl),double(maxspl)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
