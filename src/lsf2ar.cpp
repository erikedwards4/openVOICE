...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Converts LSFs back to autoregressive (AR) coeffs along rows or cols of X.\n";
    ...
    if (i1.T==1)
    {
        ...
        if (ov::lsf2ar_s(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim))
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::lsf2ar_d(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
