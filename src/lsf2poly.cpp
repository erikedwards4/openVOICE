...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Converts LSFs back to polynomials along rows or cols of X.\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ lsf2poly X -o Y \n";
    descr += "$ lsf2poly X > Y \n";
    descr += "$ cat X | lsf2poly > Y \n";
    ...
    if (i1.T==1)
    {
        ...
        if (ov::lsf2poly_s(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim))
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::lsf2poly_d(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
