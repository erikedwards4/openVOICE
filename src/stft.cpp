...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Does STFT (short-time Fourier transform) of univariate X.\n";
    ...
    descr += "If d=0, then Y is nfrqs x T [default]; if d=1, then Y is T x nfrqs,\n";
    descr += "where: \n";
    descr += "nfrqs is the number of positive FFT freqs = floor(nfft/2) + 1;\n";
    ...
    void *argtable[] = {a_fi, a_fs, a_fr, a_c0, a_mn0, a_n, a_d, a_fo, a_help, a_end};
    ...
    //Get nfft
    if (a_n->count==0) { nfft = 1; while (nfft<int(i2.N())) { nfft *= 2; } }
    ...
    o1.F = i1.F; o1.T = i1.T;
    o1.R = (dim==1) ? uint32_t(T) : uint32_t(nfft)/2+1;
    o1.C = (dim==0) ? uint32_t(T) : uint32_t(nfft)/2+1;
    o1.S = i1.S; o1.H = i1.H;
    ...
    if (i1.T==1)
    {
        ...
        if (ov::stft_s(Y,o1.iscolmajor(),int(o1.R),int(o1.C),X,int(i1.N()),W,int(i2.N()),dim,c0,float(stp),mn0,nfft))
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::stft_d(Y,o1.iscolmajor(),int(o1.R),int(o1.C),X,int(i1.N()),W,int(i2.N()),dim,c0,double(stp),mn0,nfft))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
