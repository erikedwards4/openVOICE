...
int main(int argc, char *argv[])
{
    ...
    if (i1.T==1)
    {
        ...
        if (ov::poly2roots_s(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim))
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::poly2roots_d(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101)
    {
        ...
        if (ov::poly2roots_c(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102)
    {
        ...
        if (ov::poly2roots_z(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
