...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Gets autoregressive (AR) params from reflection coeffs (RCs) along rows or cols of X.\n";
    descr += "\n";
    ...
    if (i1.T==1)
    {
        ...
        if (ov::rc2ar_s(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim))
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::rc2ar_d(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101)
    {
        ...
        if (ov::rc2ar_c(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102)
    {
        ...
        if (ov::rc2ar_z(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
