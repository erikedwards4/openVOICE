...
int main(int argc, char *argv[])
{
    ...
    ioinfo i1, i2, o1;
    int dim, g, c0, T;
    double fs, fr, stp, lvl;


    //Description
    string descr;
    descr += "Gets level crossing rate (LCR) of X along dim.\n";
    descr += "Output (Y) has a moving average of LCs using the window W.\n";
    descr += "W is the output of a window function, e.g. hamming.\n";
    descr += "\n";
    descr += "Use -v (--level) to give the level of X to test for.\n";
    descr += "For -v0 [default], this is identical to zero_crossings.\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ lcr_windowed X -o Y \n";
    descr += "$ lcr_windowed -d1 X > Y \n";
    descr += "$ cat X | lcr_windowed -g1 > Y \n";
    ...
    void *argtable[] = {a_fi, a_lvl, a_fs, a_fr, a_c0, a_d, a_g, a_fo, a_help, a_end};
    ...
    if (i1.T==1)
    {
        ...
        if (ov::lcr_windowed_s(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),W,int(i2.N()),dim,c0,float(stp),g,float(lvl)))
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::lcr_windowed_d(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),W,int(i2.N()),dim,c0,double(stp),g,double(lvl)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
