...
int main(int argc, char *argv[])
{
    ...
    for (c=0; c<iscale.size(); c++) { iscale[c] = char(tolower(iscale[c])); }
    ...
    for (c=0; c<oscale.size(); c++) { oscale[c] = char(tolower(oscale[c])); }
    ...
    if (i1.T==1)
    {
        ...
        if (ov::convert_freqs_s(X,int(i1.N()),iscale.c_str(),oscale.c_str())) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::convert_freqs_d(X,int(i1.N()),iscale.c_str(),oscale.c_str())) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
