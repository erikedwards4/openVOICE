...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Gets 1st and 2nd order differences (deltas and delta-deltas) of X along dim.\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ get_delta_deltas X -o Y \n";
    descr += "$ get_delta_deltas X > Y \n";
    descr += "$ cat X | get_delta_deltas -n4 > Y \n";
    ...
    if (i1.T==1)
    {
        ...
        if (ov::get_delta_deltas_s(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,N))
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::get_delta_deltas_d(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,N))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
