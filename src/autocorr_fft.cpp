...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Gets autocorrelation of each column or row of X using FFT method.\n";
    descr += "The means are subtracted before taking ifft(|fft|.^2).\n";
    descr += "This normalizes lag 0 of Y to 1.0 (see autocov_fft for unnormalized).\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ autocorr_fft -l127 X -o Y \n";
    descr += "$ autocorr_fft -d1 -l127 X > Y \n";
    descr += "$ cat X | autocorr_fft -l127 > Y \n";
    ...
    void *argtable[] = {a_fi, a_l, a_d, a_fo, a_help, a_end};
    ...
    if (i1.T==1)
    {
        ...
        if (ov::autocorr_fft_s(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,L)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::autocorr_fft_d(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,L)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
    ...
    else if (i1.T==101)
    {
        ...
        if (ov::autocorr_fft_c(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,L)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
    ...
    else if (i1.T==102)
    {
        ...
        if (ov::autocorr_fft_z(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,L)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
