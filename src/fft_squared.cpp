...
int main(int argc, char *argv[])
{
    ...
    descr += "and squares the output to give real-valued power.\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "The output (Y) is real (power) with size: \n";
    descr += "d=0:     nfft/2+1 x C \n";
    descr += "d=1:     R x nfft/2+1 \n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ fft_squared -n256 X -o Y \n";
    descr += "$ fft_squared -n256 -d1 X > Y \n";
    descr += "$ cat X | fft_squared -n256 > Y \n";
    ...
    //Get nfft
    if (a_n->count==0) { nfft = (dim==0) ? int(i1.R) : int(i1.C); }
    ...
    if (dim==0 && nfft<int(i1.R)) { cerr << progstr+": " << __LINE__ << errstr << "nfft must be > nrows of X" << endl; return 1; }
    if (dim==1 && nfft<int(i1.C)) { cerr << progstr+": " << __LINE__ << errstr << "nfft must be > ncols of X" << endl; return 1; }


    //Set output header info
    o1.F = i1.F; o1.T = i1.T;
    o1.R = (dim==0) ? uint32_t(nfft)/2+1 : i1.R;
    o1.C = (dim==1) ? uint32_t(nfft)/2+1 : i1.C;
    o1.S = i1.S; o1.H = i1.H;
    ...
    if (i1.T==1)
    {
        ...
        if (ov::fft_squared_s(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,nfft))
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::fft_squared_d(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,nfft))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
