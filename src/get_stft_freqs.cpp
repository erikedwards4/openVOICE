...
int main(int argc, char *argv[])
{
    ...
    void *argtable[] = {a_fs, a_nf, a_po, a_d, a_otyp, a_ofmt, a_fo, a_help, a_end};
    ...
    else if (o1.T==2)
    {
        ...
        if (ov::get_stft_freqs_d(Y,int(o1.N()),double(fs),nfft)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
