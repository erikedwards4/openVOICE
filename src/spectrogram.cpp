...
int main(int argc, char *argv[])
{
    ...
    void *argtable[] = {a_fi, a_fs, a_fr, a_c0, a_mn0, a_nfft, a_p, a_preg, a_d, a_fo, a_help, a_end};
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::spectrogram_d(Y,o1.iscolmajor(),int(o1.R),int(o1.C),X,int(i1.N()),W,int(i2.N()),H,dim,c0,double(stp),mn0,nfft,double(p),double(preg)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
