...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Gets autocorrelation of each column or row of X.\n";
    descr += "The means are subtracted before taking dot products.\n";
    descr += "This normalizes by lag 0 of autocov (see autocov for unnormalized).\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ autocorr -l127 X -o Y \n";
    descr += "$ autocorr -d1 -l127 X > Y \n";
    descr += "$ cat X | autocorr -l127 > Y \n";
    ...
    if (i1.T==1)
    {
        ...
        if (ov::autocorr_s(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,L)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::autocorr_d(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,L)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
    ...
    else if (i1.T==101)
    {
        ...
        if (ov::autocorr_c(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,L)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
    ...
    else if (i1.T==102)
    {
        ...
        if (ov::autocorr_z(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,L)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
