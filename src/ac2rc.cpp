...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Gets reflection coefficients (RCs) starting from the autocorrelation (AC).\n";
    descr += "Does Levinson-Durbin recursion of each row or col of X,\n";
    descr += "where X has the autocovariance (AC) functions in each row or col.\n";
    descr += "\n";
    ...
    if (i1.T==1)
    {
        ...
        if (ov::ac2rc_s(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::ac2rc_d(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
