...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Gets 1st and 2nd order differences (deltas and delta-deltas) of X\n";
    descr += "along dim, and appends them to X, so that Y has thrice the size of X.\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ add_delta_deltas X -o Y \n";
    descr += "$ add_delta_deltas X > Y \n";
    descr += "$ cat X | add_delta_deltas -n4 > Y \n";
    ...
    o1.F = i1.F; o1.T = i1.T;
    o1.R = (dim==0) ? i1.R : 3*i1.R;
    o1.C = (dim==1) ? i1.C : 3*i1.C;
    o1.S = i1.S; o1.H = i1.H;
    ...
    if (i1.T==1)
    {
        ...
        if (ov::add_delta_deltas_s(Y,o1.iscolmajor(),int(o1.R),int(o1.C),dim,N)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::add_delta_deltas_d(Y,o1.iscolmajor(),int(o1.R),int(o1.C),dim,N)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
