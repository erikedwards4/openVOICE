...
int main(int argc, char *argv[])
{
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ ac2poly_levdurb -p7 X -o Y -o V \n";
    descr += "$ ac2poly_levdurb -d1 -p7 X -o Y -o V \n";
    descr += "$ cat X | ac2poly_levdurb -p7 > Y \n";
    ...
    if (i1.T==1)
    {
        ...
        if (ov::ac2poly_levdurb_s(Y,V,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,P)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::ac2poly_levdurb_d(Y,V,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,P)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
