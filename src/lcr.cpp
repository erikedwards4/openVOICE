...
int main(int argc, char *argv[])
{
    ...
    void *argtable[] = {a_fi, a_lvl, a_fs, a_fr, a_c0, a_d, a_l, a_g, a_fo, a_help, a_end};
    ...
    if (i1.T==1)
    {
        ...
        if (ov::lcr_s(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),L,dim,c0,float(stp),g,float(lvl)))
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::lcr_d(Y,X,i1.iscolmajor(),int(i1.R),int(i1.C),L,dim,c0,double(stp),g,double(lvl)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
