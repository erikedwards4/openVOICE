...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Applies lifter along rows or cols of RxC input matrix X.\n";
    descr += "This is just a windowing in the cepstral domain.\n";
    descr += "\n";
    ...
    struct arg_dbl    *a_q = arg_dbln("q","Q","<dbl>",0,1,"bandwidth parameter [default=22.0]");
    struct arg_file  *a_fo = arg_filen("o","ofile","<file>",0,O,"output file (Y)");
    struct arg_lit *a_help = arg_litn("h","help",0,1,"display this help and exit");
    struct arg_end  *a_end = arg_end(5);
    void *argtable[] = {a_fi, a_d, a_q, a_fo, a_help, a_end};
    ...
    if (i1.T==1)
    {
        ...
        if (ov::lifter_s(X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,float(Q))) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::lifter_d(X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,double(Q))) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
