...
int main(int argc, char *argv[])
{
    ...
    if (i1.T==1)
    {
        ...
        if (ov::sig2poly_burg_s(Y,V,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,P,m0)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
    ...
    else if (i1.T==2)
    {
        ...
        if (ov::sig2poly_burg_d(Y,V,X,i1.iscolmajor(),int(i1.R),int(i1.C),dim,P,m0)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
