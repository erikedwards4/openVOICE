...
//To test compile:
//gcc -c poly2psd.c -O2 -std=c99 -Wall -Wextra
//clang -c poly2psd.c -O2 -std=c99 -Weverything
//g++ -c poly2psd.c -O2 -std=c++11 -Wall -Wextra
//clang++ -c poly2psd.c -O2 -std=c++11 -Weverything -Wno-old-style-cast
...
int poly2psd_s (float *Y, const float *X, const char iscolmajor, const int R, const int C, const float *V, const float *W, const int F, const int dim)
{
    ...
    //Checks
    if (R<1) { fprintf(stderr,"error in poly2psd_s: nrows X must be positive\n"); return 1; }
    if (C<1) { fprintf(stderr,"error in poly2psd_s: ncols X must be positive\n"); return 1; }
    ...
    if (dim==0)
    {
        ...
        else
        {
            for (c=0; c<C; c++)
            {
                cblas_sgemv(CblasRowMajor,CblasNoTrans,F,P,1.0f,Er,P,&X[c+C],C,0.0f,yr,1);
                cblas_sgemv(CblasRowMajor,CblasNoTrans,F,P,1.0f,Ei,P,&X[c+C],C,0.0f,yi,1);
    ...
    else if (dim==1)
    {
        if (iscolmajor)
        {
            for (r=0; r<R; r++)
            {
                cblas_sgemv(CblasColMajor,CblasNoTrans,F,P,1.0f,Er,F,&X[r+R],R,0.0f,yr,1);
                cblas_sgemv(CblasColMajor,CblasNoTrans,F,P,1.0f,Ei,F,&X[r+R],R,0.0f,yi,1);
...
int poly2psd_d (double *Y, const double *X, const char iscolmajor, const int R, const int C, const double *V, const double *W, const int F, const int dim)
{
    ...
    //Checks
    if (R<1) { fprintf(stderr,"error in poly2psd_d: nrows X must be positive\n"); return 1; }
    if (C<1) { fprintf(stderr,"error in poly2psd_d: ncols X must be positive\n"); return 1; }
    if (F<1) { fprintf(stderr,"error in poly2psd_d: F (length W) must be positive\n"); return 1; }
    ...
    if (dim==0)
    {
        ...
        else
        {
            for (c=0; c<C; c++)
            {
                cblas_dgemv(CblasRowMajor,CblasNoTrans,F,P,1.0,Er,P,&X[c+C],C,0.0,yr,1);
                cblas_dgemv(CblasRowMajor,CblasNoTrans,F,P,1.0,Ei,P,&X[c+C],C,0.0,yi,1);
    ...
    else if (dim==1)
    {
        if (iscolmajor)
        {
            for (r=0; r<R; r++)
            {
                cblas_dgemv(CblasColMajor,CblasNoTrans,F,P,1.0,Er,F,&X[r+R],R,0.0,yr,1);
                cblas_dgemv(CblasColMajor,CblasNoTrans,F,P,1.0,Ei,F,&X[r+R],R,0.0,yi,1);
...
int poly2psd_c (float *Y, const float *X, const char iscolmajor, const int R, const int C, const float *V, const float *W, const int F, const int dim)
{
    ...
    //Checks
    if (R<1) { fprintf(stderr,"error in poly2psd_c: nrows X must be positive\n"); return 1; }
    if (C<1) { fprintf(stderr,"error in poly2psd_c: ncols X must be positive\n"); return 1; }
    ...
    if (dim==0)
    {
        ...
        else
        {
            for (c=0; c<C; c++)
            {
                cblas_cgemv(CblasRowMajor,CblasNoTrans,F,P,o,E,P,&X[2*(c+C)],C,z,y,1);
    ...
    else if (dim==1)
    {
        if (iscolmajor)
        {
            for (r=0; r<R; r++)
            {
                cblas_cgemv(CblasColMajor,CblasNoTrans,F,P,o,E,F,&X[2*(r+R)],R,z,y,1);
...
int poly2psd_z (double *Y, const double *X, const char iscolmajor, const int R, const int C, const double *V, const double *W, const int F, const int dim)
{
    ...
    //Checks
    if (R<1) { fprintf(stderr,"error in poly2psd_z: nrows X must be positive\n"); return 1; }
    if (C<1) { fprintf(stderr,"error in poly2psd_z: ncols X must be positive\n"); return 1; }
    ...
    if (dim==0)
    {
        ...
        else
        {
            for (c=0; c<C; c++)
            {
                cblas_zgemv(CblasRowMajor,CblasNoTrans,F,P,o,E,P,&X[2*(c+C)],C,z,y,1);
    ...
    else if (dim==1)
    {
        if (iscolmajor)
        {
            for (r=0; r<R; r++)
            {
                cblas_zgemv(CblasColMajor,CblasNoTrans,F,P,o,E,F,&X[2*(r+R)],R,z,y,1);
